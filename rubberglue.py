...
LOGGING_FILE = '/log/rubberglue.log'
...
class Core(object):
    ...
    def logg(self, msg):
            ...
            msg=msg, time=time.strftime('%H:%M:%S %m/%d/%Y')))
        fi.close()
...
class Forwarder(asyncore.dispatcher, object):
    ...
    def handle_accept(self):
        ...
        tag = instance.make_hash('{time}---!!!---{addr}---!!!---{port}'.format(
            time=time.strftime('%H:%M:%S %m/%d/%Y'), addr=str(addr), port=str(self.port)))
        print('Connection from: {addr}:{addw}->{port}'.format(
            addr=addr[0], addw=str(addr[1]), port=str(self.port)))
        instance.logg('Connection from: {addr}:{addw}->{port}; {tag}'.format(
            addr=addr[0], addw=str(addr[1]), port=str(self.port), tag=tag))
...
class Sender(asyncore.dispatcher, object):
    def __init__(self, Receiver, remoteaddr, remoteport, tag):
        ...
        try:
            ...
            print('We are unable to forward to the remote machine on port {port}'.format(
                port=remoteport))
        self.remoteaddr = remoteaddr
        self.remoteport = remoteport

    def handle_error(self):
        print('We are unable to forward to the remote machine at {addr}:{port} (probably, their port is closed)'.format(
            addr=self.remoteaddr, port=self.remoteport))
        self.close()
